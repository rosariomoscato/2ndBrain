  ...
  const { error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family: 'Space Grotesk', sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0f; color: #e4e4e7; padding: 40px; border: 1px solid rgba(168, 85, 247, 0.3); border-radius: 16px;">
        ...
        <p style="color: #a1a1aa; font-size: 16px; line-height: 1.6;">Welcome to Second Brain. Click the button below to verify your email address and activate your account.</p>
...
export async function sendPasswordResetEmail({ to, url }: { to: string; url: string }) {
  const { error } = await resend.emails.send({
    from: FROM_EMAIL,
    to,
    subject: "Reset your password - Second Brain",
    html: `
